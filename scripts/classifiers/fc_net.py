...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # Прямой проход для первого слоя (affine - relu)
        a1, cache1 = affine_relu_forward(X, self.params['W1'], self.params['b1'])

        # Прямой проход для второго слоя (affine)
        ...
        if y is not None:
          # Вычисление потерь и градиента с использованием softmax
          loss, dscores = softmax_loss(scores, y)
          # Добавление L2 регуляризации к потерям
          loss += 0.5 * self.reg * (np.sum(self.params['W1'] ** 2) + np.sum(self.params['W2'] ** 2))

          # Обратный проход для второго слоя
          da1, dW2, db2 = affine_backward(dscores, cache2)
          dW2 += self.reg * self.params['W2']  # Добавление градиента регуляризации

          # Обратный проход для первого слоя
          dx, dW1, db1 = affine_relu_backward(da1, cache1)
          dW1 += self.reg * self.params['W1']  # Добавление градиента регуляризации

          # Сохранение градиентов в словаре grads
          grads['W1'] = dW1
          grads['b1'] = db1
          grads['W2'] = dW2
          grads['b2'] = db2
